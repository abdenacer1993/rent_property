...
if ($result == 1) {
    $msg = "User added successfully. You can login now";
    $msg = base64_encode($msg);
    $error = "false";
    $error = base64_encode($error);
    header("location:../login/login.php?result=".$msg."&error=".$error);
} elseif ($result == -1) {
    $msg = "An error occurred during the addition of this user, please try again";
    $msg = base64_encode($msg);
    $error = "true";
    $error = base64_encode($error);
    header("location:../login/create_account.php?result=".$msg."&error=".$error);
} else {
